...
// Optional offline-tuned tile + split-K table. Generated by
// tools/python/gen_sgmm_tuned_table.py from an autotuner sweep; absent by
// default, in which case SelectTiling uses the built-in heuristic. When present
// it is consulted before the heuristic.
...
// Looks up an offline-tuned tiling for this problem from the generated table.
...
// the largest bucket). Returns nullopt when no table is compiled in, the arch is
// not covered, or no entry matches.
...
// Programmatic override set by the autotuner via SetSgMatMulConfigOverride. Takes
// precedence over the tuned table and the heuristic when valid. Not thread-safe.
std::optional<SubgroupMatrixTiling> g_tiling_override;

// When true, the selector declines every MatMul so the generic WebGPU MatMul
...
// every selector invocation so the offline tuner can label its CSV.
...
SubgroupMatrixTiling SelectTiling(std::string_view arch, uint32_t M, uint32_t N, uint32_t K, uint32_t batch) {
  if (g_tiling_override && IsTilingValid(*g_tiling_override, K)) {
...
// Offline-autotuning hooks (declared in subgroup_matrix_matmul_tuning.h). Not
// used on the production inference path.
std::vector<SubgroupMatrixTiling> EnumerateSubgroupMatrixTilings(uint32_t /*M*/, uint32_t /*N*/, uint32_t K) {
  std::vector<SubgroupMatrixTiling> configs;
  for (uint32_t tile_m : kTileMCandidates) {
    for (uint32_t tile_n : kTileNCandidates) {
      for (uint32_t split_k : kSplitKCandidates) {
        const SubgroupMatrixTiling t{tile_m, tile_n, split_k};
        if (IsTilingValid(t, K)) {
          configs.push_back(SubgroupMatrixTiling{tile_m, tile_n, split_k});
        }
      }
    }
  }
