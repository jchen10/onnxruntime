...
Measurement BenchmarkConfig(const std::string& model_data,
                            const NameMLValMap& feeds,
                            const std::vector<std::string>& output_names,
                            const wgpu::SubgroupMatrixTiling& tiling) {
  Measurement m;
  m.tiling = tiling;
...
TEST(WebGpuSgMatMulTuning, SweepAndEmitCsv) {
  ...
  for (uint32_t M : kDimGrid) {
    for (uint32_t N : kDimGrid) {
      for (uint32_t K : kDimGrid) {
        const std::vector<wgpu::SubgroupMatrixTiling> tilings = wi::EnumerateSubgroupMatrixTilings(M, N, K);
        if (tilings.empty()) {
          continue;
        }
        ...
        std::cout << "[sgmm-tune] (" << problem_index << "/" << total_problems << ", "
                  ...
                  << " : benchmarking " << tilings.size() << " configs..." << std::endl;
        ...
        for (const auto& tiling : tilings) {
          const Measurement m = BenchmarkConfig(model_data, feeds, output_names, tiling);
          const double m_metric = (m.gpu_us > 0.0) ? m.gpu_us : m.wall_us;
          if (verbose) {
            std::cout << "[sgmm-tune]     tile " << tiling.tile_m << "x" << tiling.tile_n
                      << " split_k=" << tiling.split_k << " -> gpu=" << m.gpu_us
        ...
        if (have_best) {
          ...
          csv << arch << "," << M << "," << N << "," << K << ","
              << best.tiling.tile_m << "," << best.tiling.tile_n << "," << best.tiling.split_k << ","
